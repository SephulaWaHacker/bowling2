...
let this.people = [person_1, person_2, person_3];
let i = 0;

while (i < 10) {
  for (j = 0; j < this.people.length; j++){
    for (x = 0; x < 2; x++)
      this.people[j].frames[i].push(Roll());
    const sumTotal = this.people[j].frames[i].reduce((total, amount) => total + amount);
      this.people[j].frameScores.push(sumTotal)
        this.people[j].totalScore = this.people[j].frameScores.reduce((total, amount) => total + amount);
      console.log("player" + [j + 1] +" frame " + [i + 1] + " score :" + this.people[j].frames[i]); 
